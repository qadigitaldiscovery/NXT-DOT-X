...
export async function fetchWooConfig() {
    try {
...
export async function testWooConnection(config) {
    try {
...
export async function saveWooConfig(config) {
    try {
        const { data: session } = await supabase.auth.getSession();
        if (!session?.session) {
...
export async function syncWooData(config) {
    try {

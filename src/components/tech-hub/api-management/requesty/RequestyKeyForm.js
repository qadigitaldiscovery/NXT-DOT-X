...
const RequestyKeyForm = () => {
    ...
    const verifyRequestyKey = async (apiKey) => {
        try {
    ...
    return (_jsx(ApiKeyForm, { providerName: "Requesty", apiKey: apiKey, isVisible: isVisible, config: config, onApiKeyChange: setApiKey, onVisibilityToggle: () => setIsVisible(!isVisible), onConfigUpdate: handleConfigUpdate, apiKeyPlaceholder: "rty-...", docsLink: {
            text: "Requesty API Keys page",
            url: "https://requesty.ai/dashboard/api-keys"
        }, onVerify: verifyRequestyKey, preferredModelOptions: modelOptions, initialModel: "openai/gpt-4o-mini", footerText: "Your API key is stored securely and never exposed to the browser. Visit the Requesty API Keys page to create a new key if needed." }));
};

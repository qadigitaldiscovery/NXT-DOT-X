 ...
 */
export const saveToDatabase = async (providerName, apiKey, preferredModel, additionalConfig = {}) => {
    try {
 ...
 */
export const loadFromDatabase = async (providerName, defaultModel, defaultConfig = {}) => {
    try {
        // Check if user is authenticated
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {
 ...
 */
export const deleteFromDatabase = async (providerName) => {
    try {
        // Check if user is authenticated
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {

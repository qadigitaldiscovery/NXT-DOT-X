...
const SidebarMenuButton = React.forwardRef(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
    ...
    const button = (_jsx(Comp, { ref: ref, href: "#", onClick: (e) => {
            e.preventDefault();
            if (props.onClick)
                props.onClick(e);
        }, "data-sidebar": "menu-button", "data-size": size, "data-active": isActive, className: cn(sidebarMenuButtonVariants({ variant, size }), className), ...props }));
    if (!tooltip) {
        return button;
    }
    ...
    if (showTooltip) {
        return (_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: button }), _jsx(TooltipContent, { side: "right", align: "center", ...tooltip })] }));
    }
    return button;
});

...
export function FilePreview({ file, onDetectedSupplier }) {
    const [previewContent, setPreviewContent] = useState(null);
    const [extractedSupplier, setExtractedSupplier] = useState(null);
    const extractSupplierFromExcel = (jsonData) => {
        if (jsonData.length > 0) {
            const firstRow = jsonData[0];
            const headers = Object.keys(firstRow);
            // Look for supplier information in headers
            const supplierHeaderIndex = headers ?
                headers.findIndex(header => String(header).toLowerCase().includes('supplier') ||
                    String(header).toLowerCase().includes('vendor') ||
                    String(header).toLowerCase().includes('manufacturer')) : -1;
            if (supplierHeaderIndex !== -1 && firstRow) {
                ...
                if (supplierName) {
                    ...
                    if (onDetectedSupplier)
                        onDetectedSupplier(String(supplierName));
                    return true;
                }
            }
        }
    ...
    const extractSupplierFromFilename = (filename) => {
        ...
        if (possibleName && possibleName !== filename) {
            ...
            if (onDetectedSupplier)
                onDetectedSupplier(possibleName);
            return true;
        }
        return false;
    };
    const extractSupplierFromText = (lines) => {
        ...
        for (const line of lines) {
            for (const pattern of supplierIndicators) {
                ...
                if (match && match[1] && match[1].trim()) {
                    ...
                    if (onDetectedSupplier)
                        onDetectedSupplier(match[1].trim());
                    return true;
                }
            }
        }
        ...
        // For Excel files - use ExcelJS to extract and preview content
            ...
            const processExcelFile = async () => {
                try {
                    ...
                    const previewRows = jsonData.slice(0, 5).map(row => Object.values(row).join('\t'));
    ...
    if (!file)
        return null;
    return (_jsx(Card, { className: "mt-4", children: _jsxs(CardContent, { className: "pt-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx(FileTypeIcon, { fileType: file.type, fileName: file.name }), _jsx("span", { className: "font-medium", children: file.name }), _jsx(FileSize, { bytes: file.size })] }), extractedSupplier && (_jsx("div", { className: "mb-3 mt-2", children: _jsxs(Badge, { variant: "outline", className: "bg-amber-50 text-amber-800 border-amber-200", children: ["Detected Supplier: ", extractedSupplier] }) })), previewContent && (_jsxs("div", { className: "mt-3", children: [_jsx("h4", { className: "text-sm font-medium mb-1", children: "File Preview:" }), _jsx("div", { className: "bg-muted p-2 rounded-md text-xs overflow-auto max-h-40 whitespace-pre-wrap font-mono", children: previewContent })] }))] }) }));
}

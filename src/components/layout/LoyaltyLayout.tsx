...
export const LoyaltyLayout = ({
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden">
      ...
      {/* Sidebar */}
      <div className={`bg-slate-400 ${sidebarOpen ? 'w-64' : 'w-0 md:w-16'} transition-all duration-300`}>
        {/* Sidebar header */}
        <div className="flex items-center justify-between h-16 px-4 border-b">
          <div className={`flex items-center space-x-2 ${!sidebarOpen && 'md:hidden'}`}>
            ...
            <h1 className="font-bold text-xl">Loyalty Rewards</h1>
          </div>
        ...
        <nav className="p-4">
          <div className={`space-y-2 ${!sidebarOpen && 'md:flex md:flex-col md:items-center'}`}>
            <NavLink to="/loyalty-rewards" className={({
            isActive
            ...
            <NavLink to="/loyalty-rewards/members" className={({
            isActive
          }) => `flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-purple-100 text-purple-700' : 'text-gray-700 hover:bg-gray-100'} ${!sidebarOpen && 'md:justify-center md:px-2'}`}>
              {!sidebarOpen && !isMobile ? (
                <Tooltip>
            ...
            <NavLink to="/loyalty-rewards/rewards" className={({
            isActive
          }) => `flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-purple-100 text-purple-700' : 'text-gray-700 hover:bg-gray-100'} ${!sidebarOpen && 'md:justify-center md:px-2'}`}>
              {!sidebarOpen && !isMobile ? (
                <Tooltip>
            ...
            <NavLink to="/loyalty-rewards/analytics" className={({
            isActive
          }) => `flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-purple-100 text-purple-700' : 'text-gray-700 hover:bg-gray-100'} ${!sidebarOpen && 'md:justify-center md:px-2'}`}>
              {!sidebarOpen && !isMobile ? (
                <Tooltip>
            ...
            <NavLink to="/loyalty-rewards/settings" className={({
            isActive
          }) => `flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-purple-100 text-purple-700' : 'text-gray-700 hover:bg-gray-100'} ${!sidebarOpen && 'md:justify-center md:px-2'}`}>
              {!sidebarOpen && !isMobile ? (
                <Tooltip>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b flex items-center px-4 shadow-sm">
          ...
          <h1 className="text-xl font-bold">Loyalty Rewards Program</h1>
        </header>

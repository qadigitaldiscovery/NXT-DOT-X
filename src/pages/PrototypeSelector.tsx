...
import { 
  FileUp, BarChart3, Gift, ArrowDownUp, LogOut, Database, 
  Settings, FileCode, BookOpen, Link as LinkIcon, Image, Video, 
  Users, Shield, Laptop, Calendar, Mail, Search
} from 'lucide-react';
      ...
      <main className="container mx-auto px-4 py-8 flex-grow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {primaryModules.map(module => (
            <Card 
              key={module.id} 
              className="overflow-hidden transition-all duration-300 hover:shadow-xl border-0 shadow-md hover:scale-105"
            >
              <div className={`bg-gradient-to-br ${module.bgColor} py-8 flex justify-center items-center`}>
              ...
              <CardFooter className="pt-2 pb-6">
                <Button 
                  className="w-full bg-[#c01c1c] hover:bg-[#a51919] font-medium text-lg py-6" 
                  onClick={() => handlePrototypeClick(module.id, module.path)}
                >
                  Launch Module
                </Button>
              </CardFooter>
          ...
          {systemFunctions.map(system => (
            <HoverCard key={system.id}>
              <HoverCardTrigger asChild>
                <Card 
                  className="overflow-hidden transition-all duration-200 hover:shadow-md border cursor-pointer"

import { RequestyError } from './types';
import { tryUseEdgeFunction, processStreamingResponse, ApiError, getApiKey } from '../common/shared-utils';
// Get API key from storage or database
export async function getRequestyKey() {
    return await getApiKey('requesty');
}
;
// Main function to call Requesty API
export async function callRequesty({ endpoint, payload, apiKey, signal }) {
    ...
    try {
        const edgeResponse = await tryUseEdgeFunction('requesty', endpoint, payload);
        if (edgeResponse) {
...
export async function sendRequestyMessage(messages, model) {
    // Ensure messages is an array
    const messageArray = Array.isArray(messages) ? messages : [messages];
    try {
        const response = await callRequesty({
...
export async function* streamRequestyMessage(messages, model) {
    // Ensure messages is an array
    const messageArray = Array.isArray(messages) ? messages : [messages];
    try {
        // Fix: Properly type the response as ReadableStream instead of RequestyResponse
        const stream = await callRequesty({
            endpoint: 'chat/completions',
            payload: {
...
export async function routeAIRequest({ model, messages, temperature, maxTokens, apiKey, signal }) {
    const response = await callRequesty({
        endpoint: 'chat/completions',
        payload: {
            model,

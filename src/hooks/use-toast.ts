...
interface ExtendedToastFunction {
  ...
  success: (message: string | Omit<ToastProps, "variant">) => string | number;
  error: (message: string | Omit<ToastProps, "variant">) => string | number;
  ...
  return {
    toast: ((props: ToastProps) => {
      ...
      if (props.variant === "destructive") {
        return sonnerToast.error(props.title as string || "Error", {
      ...
      return sonnerToast(props.title as string, {
        ...
        action: props.action
          ? {
              label: "Action",
              onClick: () => {
                // The ToastAction component should handle its own onClick
              },
            }
          : undefined,
      });
    }) as ExtendedToastFunction,
    // Return empty array for compatibility with the Toaster component
    toasts: [] as Toast[],
  };
};
...
currentToastInstance.error = (message: string | Omit<ToastProps, "variant">) => {
  if (typeof message === "string") {
    ...
    return sonnerToast.error(message.title as string || "Error", {
      description: message.description,
      action: message.action
        ? {
            label: "Action",
...
// Add dismiss method
currentToastInstance.dismiss = (toastId?: string | number) => {
